...
// Default CLI settings and the external API endpoints used for discovery.
...
// Print CLI usage instructions to standard error.
function printUsage() {
  console.error(
...
// CLI entry point: parse arguments, run the search, and output JSON.
async function main() {
...
// Search all sources in parallel and return normalized candidates plus summary data.
async function searchMcpServers(options = {}) {
  ...
  // Accumulate results while preserving per-source success or failure details.
...
// Group normalized candidates by canonical key and collapse duplicates.
...
// Normalize Official MCP Registry results into the shared schema.
...
// Normalize Smithery results into the shared schema.
...
// Normalize GitHub repository search results into the shared schema.
...
// Build the request definitions for each upstream source.
...
// Execute an HTTP request and surface JSON parsing failures with context.
...
// Build the search URL for the Registry API.
...
// Build the search URL for the Smithery API.
...
// Build the search URL for the GitHub Search API.
...
// Create the initial merged record for a single candidate.
...
// Merge additional source data into an existing combined candidate.
...
// Sort final candidates by popularity and usage signals.
...
// Derive a canonical key from repository or homepage data for deduplication.
...
// Infer the installation method from Registry metadata.
...
// Return the first usable URL from the remote definitions.
...
// Walk nested package metadata recursively to find an npm package name.
...
// Extract the trailing display name from an owner/name style identifier.
...
// Extract only a GitHub repository URL from an arbitrary URL.
...
// Normalize URL variations such as trailing slashes and .git suffixes.
...
// Ignore bare top-level homepages and keep only meaningful URLs.
...
// Clamp the limit value to a safe integer range.
...
// Accept numeric fields only when they are finite numbers.
function normalizeNumber(value) {
  return typeof value === 'number' && Number.isFinite(value) ? value : null;
}

// Prefer the more specific and useful value when merging candidates.
...
// Report unexpected failures and exit with a non-zero status.
main().catch((error) => {
